...
// 全局变量，用于存储ADC转换结果
uint16_t adc_values[3]; // 存储三个通道的转换结果
...
void HAL_ADC_MspInit(ADC_HandleTypeDef* adcHandle)
{
  ...
  /* USER CODE END ADC1_MspInit 0 */
    ...
    PA6     ------> ADC1_IN6
    */
    GPIO_InitStruct.Pin = LIGHT_Pin|MQ2_Pin;
...
void HAL_ADC_MspDeInit(ADC_HandleTypeDef* adcHandle)
{
  ...
  /* USER CODE END ADC1_MspDeInit 0 */
    ...
    PA6     ------> ADC1_IN6
    */
    HAL_GPIO_DeInit(GPIOA, LIGHT_Pin|MQ2_Pin);
...
uint32_t ADC_ReadChannel(uint32_t channel)
{
    ...
    if(channel == ADC_CHANNEL_4)
        return adc_values[0];  // LIGHT
    else if(channel == ADC_CHANNEL_6)
        return adc_values[1];  // MQ2
